...
const PrivacyPolicy = () => {
  ...
  const currentData = privacyPolicyData[language] || privacyPolicyData.en;
  ...
  return (
    <div className="legal-page-root privacy-policy-page">
      <Navbar />
                ...
                {currentData.sections.map((sec, index) => (
                  <div key={sec.id}>
                    <section id={sec.id} className="legal-content-section">
                      ...
                      {sec.content.map((pText, pIndex) => (
                        <p key={pIndex}>{pText}</p>
                      ))}

                      {/* Subsections if present (Comments & Reviews, Media Uploads) */}
                      {sec.subsections && sec.subsections.length > 0 && (
                        <div className="legal-subsections-group">
                          {sec.subsections.map((sub, subIdx) => (
                            <div key={subIdx} className="legal-subsection-block">
                              <h3>{sub.title}</h3>
                              <p>{sub.text}</p>
                            </div>
                          ))}
