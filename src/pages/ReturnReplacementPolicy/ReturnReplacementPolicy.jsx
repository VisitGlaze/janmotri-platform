...
import { returnReplacementPolicyData } from "./returnReplacementPolicyData";
...
const ReturnReplacementPolicy = () => {
  ...
  const currentData = returnReplacementPolicyData[language] || returnReplacementPolicyData.en;
  ...
  return (
    <div className="legal-page-root return-replacement-policy-page terms-conditions-page">
      <Navbar />
                ...
                {currentData.sections.map((sec, index) => (
                  <div key={sec.id}>
                    <section id={sec.id} className="legal-content-section">
                      ...
                      {/* Numbered List if present (e.g. Return Process) */}
                      {sec.numbered && sec.numbered.length > 0 && (
                        <ol className="terms-numbered-list">
                          {sec.numbered.map((nText, nIdx) => (
                            <li key={nIdx}>{nText}</li>
                          ))}
                        </ol>
                      )}
